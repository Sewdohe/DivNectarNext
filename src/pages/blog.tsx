      ...
      <div className="py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-3xl font-bold tracking-tight text-violet-400 sm:text-4xl">DivNectar Blog</h2>
              ...
              <article key={post.id} className="flex flex-col items-start justify-between">
                ...
                <div className="max-w-xl">
                  <div className="mt-8 flex items-center gap-x-4 text-xs">
                    <time dateTime={post.frontmatter?.date!} className="text-subText">
                      {post.frontmatter?.date}
                    </time>
                    {post.frontmatter?.category?.map(cat => (
                      <a
                        href={post.frontmatter?.author!}
                        className="relative z-10 rounded-full bg-peach px-3 py-1.5 font-medium text-black hover:bg-gray-100"
                      >
                        {cat}
                      </a>
                    ))}
